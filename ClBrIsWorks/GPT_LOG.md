...
队友的根 `main.c` 最简巡迹代码已经完成上车验证。正式分层代码以该行为
为准，不再使用错误的连续标定端点：

```text
五路阈值：330、295、290、300、500
黑线极性：五路均为 raw >= threshold
直行轮输出：220、220
左微调轮输出：140、210
右微调轮输出：210、140
左急转轮输出：110、230
右急转轮输出：230、110
丢线搜索：沿最后偏移方向使用 110、230 或 230、110
停止标志：L1、C、R1 同时检测到黑线时制动
```

`line_sensor` 同时保留两种明确模式：

```text
LINE_SENSOR_MODE_BINARY_THRESHOLD：使用逐路阈值输出 0 或 1000
LINE_SENSOR_MODE_CALIBRATED：具有完整白底/黑线端点后使用连续归一化
```

当前正式应用使用 `LINE_SENSOR_MODE_BINARY_THRESHOLD`。没有实测到完整端点
之前，禁止从阈值反推并填写虚假的 `backgroundValue` 和 `lineValue`。

启动和安全语义：

```text
初始化成功：立即进入 APPLICATION_RUNNING 并开始巡迹
s：不再是控制命令
x：立即制动并进入 APPLICATION_IDLE
x 后恢复：复位主控
ADC 故障：立即制动
尚无历史偏移时丢线：立即制动
已有历史偏移时丢线：沿最后方向搜索，重新检测到线后自动恢复 RUNNING
```
