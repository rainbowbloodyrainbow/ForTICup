...
速度环输出的是对称倾斜需求 `u`，不是电机角度。正负方向分别映射，从而补偿丝杠机构两侧角度—倾斜度不对称。倾斜需求限制为±1500；当前源码又把最终视觉电机目标硬限制在 `-30°～+20°`。
...
BALL_TILT_U_LIMIT_MILLI             // 对称倾斜需求限幅±1500
VISION_MIN/MAX_TARGET_MILLIDEG      // 当前最终电机限幅-30°/+20°
```
...
立即切换为位置目标x=135的位置P+速度P闭环
  ↓
x在135±2且|速度|≤10 pixel/s连续300ms
  ↓
REQ3 COMPLETE，继续闭环保持在x=135
```

切换时不会清零加速阶段测得的球速，因此PID能够根据小球仍在向右运动的速度立即反向制动。串口会依次输出：
